...
const order = {
  ...
  str() {
    let str = '';
    if (this.total() === 0) str = 'Your Order list is empty!';
    else {
      for (i in this.items) {
...
function getBalance() {
  return Number(
    JSON.parse(document.getElementById('balance-data').textContent)
  );
}
...
function updateDate() {
  // update date in html
  document.querySelector('#cal-month').innerHTML = date
    .tommorow()
    .toDateString()
    .slice(4, 7); //initialize cal-order month
  document.querySelector('#cal-day').innerHTML = (
    '0' + date.tommorow().getDate()
  ).slice(-2); //initialize cal-order day
}
function updateOrderHTML() {
  // update order in html
  document.querySelector('#text-order').innerHTML = order.str();
  if (order.total() > 0) {
    // if items selected
    document.querySelector('#total-order').innerHTML =
      '<span>Total: ' + order.total() + ' AED</span>'; // update total
    document.querySelector('#discounted').innerHTML =
      order.discounted() + ' AED'; // update discount
    document.querySelector('#total-TBP').innerHTML =
      'To be paid: ' + order.TBP() + ' AED'; // update discount
...
function updateQty(HTMLObj) {
  // update qty in html
  if (HTMLObj.qty) HTMLObj.qty.innerText = window[HTMLObj.name].qty;
}
function updateQtyDefault(HTMLObj) {
  // update qty in html
  if (HTMLObj.qty) HTMLObj.qty.innerText = window[`${HTMLObj.name}_def`].qty;
}
// global functions
...
function geocodeToString(obj) {
  ...
  for (let i = 0; i < obj.length; i++) {
    for (let j = 0; j < obj[i].types.length; j++) {
      obj[i].types[j] === 'street_number'
        ? (address += obj[i].long_name + ', ')
        : '';
...
function pay(balance) {
  ...
  if (order.TBP() - balance > 0 && balance >= 0) {
    //if remaining is positive
    let c = confirm(
      `Your remaining balance (${balance} AED) is not enough. Do you confirm the CB payment for the remaining amount of ${remaining} AED?`
    ); //
    if (c) {
      handler.open({
        currency: 'aed',
    ...
    let c = confirm(
      `${order.TBP()} AED will be debited from your current Robert balance. Do you confirm the payment?`
    );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.title === 'Robert - Best Bread Delivery') {
    // index
    setAutoComplete();
    $('#address').keypress(
      // prevent enter to sbmit the form and click button instead
      function(event) {
        if (event.which == '13') {
  ...
  if (document.title === 'Robert - Order') {
    // order
    hide('.checkBox-load');
    updateDate();
    ...
    itemHtml.forEach(i => {
      ...
      try {
        ...
        window[HTMLObj.name] = new Item(
          savedItem.type,
          savedItem.name,
          getItemPrice(HTMLObj.name),
          savedItem.qty
        );
      } catch (err) {
        // otherwize initialize a new item object
        console.log(err);
        window[HTMLObj.name] = new Item(
          HTMLObj.type,
          HTMLObj.name,
          HTMLObj.price,
          HTMLObj.qty.innerHTML
        );
      }
  ...
  if (
    document.title === 'Robert - Confirm' ||
    document.title === 'Robert - Account'
  ) {
    // confirm
    setAutoComplete();
    const geocoder = new google.maps.Geocoder();
    ...
    try {
      ...
      console.log(err);
      if (document.getElementById('user-geodata').textContent) {
        // check if  dataloc in user DB
        geocode = JSON.parse(
          document.getElementById('user-geodata').textContent
        );
        localStorage.address = JSON.stringify(geocode);
      } else {
        geocoder.geocode(
          { ['location']: { lat: 25.1714393, lng: 55.22058549999997 } },
  ...
  if (document.title === 'Robert - Confirm') {
    // confirm
  ...
  if (document.title === 'Robert - Account') {
    ...
    itemHtml.forEach(i => {
      ...
      HTMLObj.price = Number(
        i.previousElementSibling.previousElementSibling.value
      );
      ...
      window[`${HTMLObj.name}_def`] = new Item_default(
        HTMLObj.type,
        HTMLObj.name,
        HTMLObj.price,
        HTMLObj.qty.innerHTML
    ...
    try {
      ...
      button_DO.onclick = () => {
        document.querySelector('#default_order').value = JSON.stringify(
          default_order
        );
        form_DO.submit();
      };
    ...
    function numberOfMonth() {
      ...
      const number_day_current_month =
        new Date(year, month, 0).getDate() - date.tommorow().getDate();
      ...
      $('#datepicker').multiDatesPicker({
        ...
        onSelect: function(d, t) {
          if (d === date.formatDp()) {
            ...
            confirm(form.dataset.message)
              ? document.getElementById('action_tomorrow_select').submit()
              : '';
          }
        }
      });
